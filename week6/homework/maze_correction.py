...
CELL_DIM = 60
x_cell_count = MAP_WIDTH // CELL_DIM 	#Số cột = 600 / 60pixel = 10
y_cell_count = MAP_HEIGHT // CELL_DIM	#Số cột = 600 / 60pixel = 10	
...
maze : list[list[Cell]] = []
for i in range(x_cell_count): # tạo từng cột
	maze.append([])
      
	for j in range(y_cell_count): #Tạo từng ô
		maze[i].append(Cell())	
...
# -------------------------------------------------
# Link Neighbours (using while loops)
# -------------------------------------------------
#kiểm tra mỗi ô có bị chặn hướng nào do nằm ở mép grid.
col = 0
...
while col < x_cell_count:
    ...
    while row < y_cell_count:
        ...
        print(
            f"Cell x: {col}, y: {row} "
            f"north:{0 if cell.north is None else 1} "
            f"south: {0 if cell.south is None else 1} "
            f"east: {0 if cell.east is None else 1} "
            f"west: {0 if cell.west is None else 1}"
        )
